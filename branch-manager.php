...
if ($action === "reset") {
    ...
    $r2 = runCmd("sudo -u bitnami bash -c \"cd " . escapeshellarg($apiDir) . " && git fetch origin && git checkout main && git reset --hard origin/main && npm ci && npm run build && npm prune --omit=dev\"");
...
if ($authenticated && isset($_GET['action'])) {
    ...
    if ($action === 'switch') {
        ...
        if ($apiBranch !== '' && validateBranch($apiBranch)) {
            ...
            $r = runCmd('sudo -u bitnami bash -c "cd ' . escapeshellarg($API_DIR) . ' && git fetch origin && git checkout ' . $esc . ' && git reset --hard origin/' . $esc . ' && npm ci && npm run build && npm prune --omit=dev"');
...
<body>
    ...
    function assignLanes(commits) {
      ...
      // Build immutable node objects with lane/row assignments
      const nodes = commits.map((c, i) => ({
        ...c,
        lane: laneMap.get(c.hash),
        row: i,
      }));
    ...
    function renderCommitTree(containerId, commits, repoLabel) {
      ...
      // Draw commit dots (with tooltips) and ref labels
      for (const node of nodes) {
        const x = laneX(node.lane);
        const y = rowY(node.row);
