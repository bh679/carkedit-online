...
function renderItem(pack, on, isHost, state, disabledSet) {
  ...
  const control = `<div class="pack-selector__controls">${previewIconBtn}${toggleBtn}</div>`;

  // Sub-row: deck chips only (preview moved up next to On/Off)
  let subRow = '';
  if (on) {
 ...
 * Host sees toggle buttons + per-deck sub-toggles + inline preview;
...
export function render(state, { isHost } = { isHost: false }) {
  ...
  // Compute per-deck totals honoring per-pack deck enablement
  ...
  const items = allPacks.map((p) => renderItem(p, selected.has(p.id), true, state, disabledSet)).join('');
